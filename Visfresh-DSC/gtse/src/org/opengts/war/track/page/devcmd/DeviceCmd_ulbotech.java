...
// ----------------------------------------------------------------------------
// Change History:
//  2015/01/15  Martin D. Flynn
//     -Initial release (cloned from "gos777")
// ----------------------------------------------------------------------------
...
public class DeviceCmd_ulbotech
    implements DeviceCmdHandler
{
    ...
    /* device code */
    public static final String  DEVICE_CODE                 = DCServerFactory.ULBOTECH_NAME;
        ...
        throws IOException
    {
        ...
        I18N         i18n        = privLabel.getI18N(DeviceCmd_ulbotech.class);
        ...
        out.println(DeviceInfo.FormRow_TextField(PARM_DEVICE  , false, i18n.getString("DeviceCmd_ulbotech.deviceID","{0} ID",devTitles)+":"            , selDevID, 30, 30));
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_DESC, false, i18n.getString("DeviceCmd_ulbotech.deviceDesc","{0} Description",devTitles) +":", (selDev!=null)?selDev.getDescription():"", 40, 40));
        ...
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_LAST_CONNECT, false, i18n.getString("DeviceCmd_ulbotech.lastCommunication","Last Communication") +":", lastCommTime , 30, 30));
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_LAST_EVENT  , false, i18n.getString("DeviceCmd_ulbotech.lastEvent","Last Event") +":"                , lastEventTime, 30, 30));

        /* commands */
        out.println("<tr class='deviceCommandList'>");
        out.println("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap style=\"vertical-align:top; padding-top:5px;\">"+i18n.getString("DeviceCmd_ulbotech.commandSelect","Command Select")+":</td>");
        ...
        if (dcs != null) {
            ...
            if (!ListTools.isEmpty(cmdMap_admin)) {
                cmdCount += this._writeCommands(out, reqState, 
                    i18n.getString("DeviceCmd_ulbotech.standardCommands","Standard Commands"),
            ...
            if (!ListTools.isEmpty(cmdMap_sysadmin)) {
                cmdCount += this._writeCommands(out, reqState, 
                    i18n.getString("DeviceCmd_ulbotech.sysadminCommands","SysAdmin Commands"),
        ...
        if (cmdCount == 0) {
            ...
            out.print("<span>"+i18n.getString("DeviceCmd_ulbotech.noCommandsAvailable","No Commands Available")+"</span>");
            out.print("</td>");
            out.print("</tr>\n");
        }
        ...
        out.println(DeviceInfo.FormRow_ComboBox(PARM_COMMAND_SELECT , editProps, i18n.getString("DeviceCmd_ulbotech.commands","Command")+":", commandSel, commandMap, "", -1));
        */
        ...
        if (editProps) {
            out.write("<input type='submit' name='"+PARM_SUBMIT_SEND+"' value='"+i18n.getString("DeviceCmd_ulbotech.send","Send")+"'>\n");
            out.write("<span style='padding-left:10px'>&nbsp;</span>\n");
            out.write("<input type='button' name='"+PARM_BUTTON_CANCEL+"' value='"+i18n.getString("DeviceCmd_ulbotech.cancel","Cancel")+"' onclick=\"javascript:openURL('"+actionURL+"','_self');\">\n");
        } else {
            out.write("<input type='button' name='"+PARM_BUTTON_BACK+"' value='"+i18n.getString("DeviceCmd_ulbotech.back","Back")+"' onclick=\"javascript:openURL('"+actionURL+"','_self');\">\n");
        }
        out.write("</form>\n");
        return true;
    ...
    {
        if (resp == null) {
            return i18n.getString("DeviceCmd_ulbotech.unableToQueueCommand","Unable to queue command for transmission");
        } else {
            ...
            if ((rc == null) || rc.isSuccess()) {
                return i18n.getString("DeviceCmd_ulbotech.commandQueued","Requested command has been queued for transmission");
            } else {
                return i18n.getString("DeviceCmd_ulbotech.commandError" ,"Unable to queue command: {0}", m);
            }
        }
    }
    ...
    public String handleDeviceCommands(RequestProperties reqState, Device selDev)
    {
        ...
        /* supported device? */
        if (!this.deviceSupportsCommands(selDev)) {
            return i18n.getString("DeviceCmd_ulbotech.doesNotSupport","Device does not support {0}", this.getServerDescription());
        }

        /* get selected command */
        String cmdSel = AttributeTools.getRequestString(request, PARM_COMMAND_SELECT, "");
        if (StringTools.isBlank(cmdSel)) {
            return i18n.getString("DeviceCmd_ulbotech.noCommandSelected","No command selected");
        }
        ...
        /* unknown command */
        return i18n.getString("DeviceCmd_ulbotech.unknownCommand","No command selected");
