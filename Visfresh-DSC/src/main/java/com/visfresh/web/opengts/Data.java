...
// ----------------------------------------------------------------------------
package com.visfresh.web.opengts;
    ...
    extends CommonServlet
{
    ...
    // ------------------------------------------------------------------------
    private static final long serialVersionUID = 1813556229138443266L;
    ...
    private static void printHeaderLine(final String desc, final String opt[], final String conf)
    {
        final String val = StringTools.join(opt,",");
        Data.printHeaderLine(desc, val, conf);
    }

    private static void printHeaderLine(final String desc, final String val, final String conf)
    {
        final int dLen = 23;
        final int vLen = 23;
        final String d = StringTools.trim(desc);
        final String v = StringTools.trim(val);
        final String c = StringTools.trim(conf);
        final StringBuffer sb = new StringBuffer();
        sb.append(StringTools.leftAlign(d,dLen)).append(": ");
        if (StringTools.isBlank(c)) {
            sb.append(v);
        } else {
            final String va = (!StringTools.isBlank(v) && !v.equals("&="))? v : "---";
    ...
    static {
        ...
        if (ListTools.isEmpty(UniqueIDPrefix)) {
            ...
            final String logName_ = LOG_NAME + "_";
        ...
        final String dateFmt     = RTConfig.getString( CONFIG_DATE_FORMAT      , "YMD");
        ...
        final String charEnc     = StringTools.getCharacterEncoding();
    ...
    private static int TranslateStatusCode(final String statusCodeStr)
    {
        ...
        final String sc = statusCodeStr.toUpperCase();

        /* check status code name */
        final StatusCodes.Code code = StatusCodes.GetCode(sc,null);
        if (code != null) {
            return code.getCode();
        }
        ...
        /* check default codes */
        final Integer sci = StatusCodeMap.get(sc);
        if (sci != null) {
            return sci.intValue();
        }
    ...
    **/
    private Device loadDevice(final String ipAddr,
        String mobileID,
        ...
        final String authCode)
    {
        ...
        if (!StringTools.isBlank(mobileID)) {
            ...
            for (int i = 0; (i < UniqueIDPrefix.length) && (device == null); i++) {
                final String uid = UniqueIDPrefix[i] + mobileID; // ie: "gprmc_123456789012345"
                try {
            ...
            if (device == null) {
                final String uid = UniqueIDPrefix[0] + mobileID;
            ...
            try {
                ...
                final String uid = accountID + "/" + deviceID;
        ...
        final DataTransport dataXPort = device.getDataTransport();
    ...
    protected void _doWork(final boolean isPost, final HttpServletRequest request, final HttpServletResponse response)
        throws ServletException, IOException
    {
        final String ipAddr     = request.getRemoteAddr();
        final String phoneID    = AttributeTools.getRequestString(request, PARM_PHONE     , null);
        final String mobileID   = AttributeTools.getRequestString(request, PARM_MOBILE    , phoneID);
        ...
        final String password   = AttributeTools.getRequestString(request, PARM_PASSWORD  , "");  // not currently used
        final String authCode   = AttributeTools.getRequestString(request, PARM_AUTH      , "");
        final String driverID   = AttributeTools.getRequestString(request, PARM_DRIVER    , "");
        String uniqueID   = null;

        /* URL */
        final StringBuffer reqURL = request.getRequestURL();
        final String queryStr = StringTools.blankDefault(request.getQueryString(),"(n/a)");
        if (isPost) {
            // 'queryStr' is likely not available
            final StringBuffer postSB = new StringBuffer();
            for (final java.util.Enumeration<?> ae = request.getParameterNames(); ae.hasMoreElements();) {
                if (postSB.length() > 0) { postSB.append("&"); }
                final String ak = (String)ae.nextElement();
                final String av = request.getParameter(ak);
                postSB.append(ak + "=" + av);
            }
        ...
        /* "&cmd=version"? */
        if (isPost) {
            final String cmd = AttributeTools.getRequestString(request, PARM_COMMAND, "");
            if (StringTools.isBlank(cmd)) {
                // -- no command requested (fixed 2.5.4-B10)
            } else
            if (ListTools.contains(PARM_VERSION,cmd)) {
                // -- version requested
                final String vers = DEVICE_CODE+"-"+VERSION;
        ...
        /* Device */
        final Device device = this.loadDevice(ipAddr, mobileID, accountID, deviceID, authCode);
        if (device == null) {
        ...
        /* message */
        final String message     = AttributeTools.getRequestString(request, PARM_MESSAGE     , "");
        final String emailAddr   = AttributeTools.getRequestString(request, PARM_EMAIL       , "");

        /* extract fields from URL arguments */
        final String _version    = AttributeTools.getRequestString(request, PARM_VERSION     , null);
        final String _statusStr  = AttributeTools.getRequestString(request, PARM_STATUS      , null);
        final String _address    = AttributeTools.getRequestString(request, PARM_ADDRESS     , null);
        final String _altitudeM  = AttributeTools.getRequestString(request, PARM_ALTITUDE    , null);
        final String _odometerKM = AttributeTools.getRequestString(request, PARM_ODOMETER_KM , null);
        final String _odometerMI = AttributeTools.getRequestString(request, PARM_ODOMETER_MI , null);
        final String _distanceKM = AttributeTools.getRequestString(request, PARM_DISTANCE_KM , null);
        final String _distanceMI = AttributeTools.getRequestString(request, PARM_DISTANCE_MI , null);
        final String _gprmcStr   = AttributeTools.getRequestString(request, PARM_GPRMC       , null);
        final String _dateStr    = AttributeTools.getRequestString(request, PARM_DATE        , null);
        final String _timeStr    = AttributeTools.getRequestString(request, PARM_TIME        , null);
        final String _latitude   = AttributeTools.getRequestString(request, PARM_LATITUDE    , null);
        final String _longitude  = AttributeTools.getRequestString(request, PARM_LONGITUDE   , null);
        final String _speedKPH   = AttributeTools.getRequestString(request, PARM_SPEED_KPH   , null);
        final String _speedMPH   = AttributeTools.getRequestString(request, PARM_SPEED_MPH   , null);
        final String _headingDeg = AttributeTools.getRequestString(request, PARM_HEADING     , null);
        final String _hdop       = AttributeTools.getRequestString(request, PARM_HDOP        , null);
        final String _horzAcc    = AttributeTools.getRequestString(request, PARM_HORZ_ACC    , null);
        final String _vertAcc    = AttributeTools.getRequestString(request, PARM_VERT_ACC    , null);
        final String _numSats    = AttributeTools.getRequestString(request, PARM_NUM_SATS    , null);
        final String _battLevel  = AttributeTools.getRequestString(request, PARM_BATT_LEVEL  , null);
        final String _battVolts  = AttributeTools.getRequestString(request, PARM_BATT_VOLTS  , null);
        final String _battTempC  = AttributeTools.getRequestString(request, PARM_BATT_TEMPC  , null);
        final String _tempC      = AttributeTools.getRequestString(request, PARM_TEMPERATURE , null);
        final String _cellMCC    = AttributeTools.getRequestString(request, PARM_CELLID_MCC  , null);
        final String _cellMNC    = AttributeTools.getRequestString(request, PARM_CELLID_MNC  , null);
        final String _cellLAC    = AttributeTools.getRequestString(request, PARM_CELLID_LAC  , null);
        final String _cellCID    = AttributeTools.getRequestString(request, PARM_CELLID_CID  , null);
        final String _cellTAV    = AttributeTools.getRequestString(request, PARM_CELLID_TAV  , null);
        final String _cellRAT    = AttributeTools.getRequestString(request, PARM_CELLID_RAT  , null);
        final String _cellRXLEV  = AttributeTools.getRequestString(request, PARM_CELLID_RXLEV, null);
        final String _cellARFCN  = AttributeTools.getRequestString(request, PARM_CELLID_ARFCN, null);
        ...
        final String statusStr   = _statusStr;
        ...
        final double odometerMI  = StringTools.parseDouble(_odometerMI, 0.0);         // miles
        double distanceKM  = StringTools.parseDouble(_distanceKM, 0.0);         // kilometers
        final double distanceMI  = StringTools.parseDouble(_distanceMI, 0.0);         // miles
        String gprmcStr    = (_gprmcStr != null)? _gprmcStr.toUpperCase() : null;// $GPRMC
        final String dateStr     = _dateStr;
        final String timeStr     = _timeStr;
        ...
        final double speedMPH    = StringTools.parseDouble(_speedMPH  , 0.0);         // mph
        double headingDeg  = this._parseHeading(_headingDeg);                   // degrees
        final double hdop        = StringTools.parseDouble(_hdop      , 0.0);         // HDOP
        double horzAcc     = StringTools.parseDouble(_horzAcc   , 0.0);         // meters
        final double vertAcc     = StringTools.parseDouble(_vertAcc   , 0.0);         // meters
        ...
        final boolean isUSUnits = AttributeTools.hasRequestAttribute(request,"usunits") ||
        ...
        final double battVolts   = StringTools.parseDouble(_battVolts , 0.0);         // volts
        final double battTempC   = StringTools.parseDouble(_battTempC , BAD_TEMP);    // degrees C
        ...
        /* temperature */
        final double tempC       = StringTools.parseDouble(_tempC     , BAD_TEMP);// degrees C

        /* cell-tower */
        final int    cellMCC     = StringTools.parseInt(_cellMCC  , 0);
        final int    cellMNC     = StringTools.parseInt(_cellMNC  , 0);
        final int    cellLAC     = StringTools.parseInt(_cellLAC  , 0);
        final int    cellCID     = StringTools.parseInt(_cellCID  , 0);
        final int    cellTAV     = StringTools.parseInt(_cellTAV  , 0);
        final int    cellRAT     = StringTools.parseInt(_cellRAT  , 0);
        final int    cellRXLEV   = StringTools.parseInt(_cellRXLEV, 0);
        final int    cellARFCN   = StringTools.parseInt(_cellARFCN, 0);
        ...
        final int statusCode = Data.TranslateStatusCode(statusStr);
        ...
        if (!StringTools.isBlank(gprmcStr)) {
            ...
            final boolean ignoreChecksum = (gprmcStr.indexOf("*") >= 0)?  // ignore checksum if not present
                false : // found, do not ignore
                true  ; // not found, ignore
            final Nmea0183 gprmc = new Nmea0183(gprmcStr, ignoreChecksum);
        ...
        if (SIMEVENT_GEOZONES && isValidGPS) {
            final java.util.List<Device.GeozoneTransition> zone = device.checkGeozoneTransitions(fixtime, geoPoint);
            if (zone != null) {
                for (final Device.GeozoneTransition z : zone) {
                    final EventData.Key zoneKey = new EventData.Key(accountID, deviceID, z.getTimestamp(), z.getStatusCode());
                    final EventData zoneEv = zoneKey.getDBRecord();
        ...
        if ((statusCode != StatusCodes.STATUS_LOCATION) || (eventCount == 0)) {
            final EventData.Key evKey = new EventData.Key(accountID, deviceID, fixtime, statusCode);
        ...
        try {
            ...
            if (!StringTools.isBlank(version)) {
                final String V = (version.length() > 30)? version.substring(0,30) : version;
    ...
    private long _parseFixtime(String dateStr, String timeStr)
    {
        ...
        if (DateFormat == DATE_FORMAT_EPOCH) {
            final String epochStr = !StringTools.isBlank(dateStr)? dateStr : timeStr;
        ...
        if (((DateFormat == DATE_FORMAT_YMDhms) || (DateFormat == DATE_FORMAT_YMD)) &&
            ...
            try {
                final DateTime dt = DateTime.parseArgumentDate(dtStr, gmtTimeZone,
                    DateTime.DefaultParsedTime.CurrentTime);
                return dt.getTimeSec();
            } catch (final DateTime.DateParseException dpe) {
        ...
        final int dateLen = dateStr.length();
        ...
        /* parse time */
        final int hh = StringTools.parseInt(timeStr.substring(0,2), 0);
        final int mm = StringTools.parseInt(timeStr.substring(2,4), 0);
        final int ss = StringTools.parseInt(timeStr.substring(4,6), 0);

        /* return epoch time */
        final DateTime dt = new DateTime(gmtTimeZone, YYYY, MM, DD, hh, mm, ss);
        return dt.getTimeSec();

    }

    private double _parseLatitude(final String latStr)
    {
        // Possible formats:
        //  -39.12345    - Decimal
        ...
        if ((latStr.length() >= 6) && Character.isDigit(latStr.charAt(0)) && (latStr.charAt(4) == '.')) {
            ...
            final double _lat = StringTools.parseDouble(latStr, 9001.0);
            if (_lat < 9000.0) {
                double lat = (long)_lat / 100L; // _lat is always positive here
                lat += (_lat - (lat * 100.0)) / 60.0;
                final char d = Character.toUpperCase(latStr.charAt(latStr.length() - 1)); // last character N/S
                return (d == 'S')? -lat : lat;
            } else {
    ...
    private double _parseLongitude(final String lonStr)
    {
        // Possible formats:
        //  142.12345       - Decimal
        ...
        if ((lonStr.length() >= 7) && Character.isDigit(lonStr.charAt(0)) && (lonStr.charAt(5) == '.')) {
            ...
            final double _lon = StringTools.parseDouble(lonStr, 18001.0);
            if (_lon < 18000.0) {
                double lon = (long)_lon / 100L; // _lon is always positive here
                lon += (_lon - (lon * 100.0)) / 60.0;
                final char d = Character.toUpperCase(lonStr.charAt(lonStr.length() - 1)); // last character E/W
                return (d == 'W')? -lon : lon;
            } else {
                return INVALID_LATLON;
            }
    ...
    private double _parseHeading(final String headingStr)
    {
        ...
        if (Character.isLetter(headingStr.charAt(0))) {
            ...
            final int ndx = ListTools.indexOfIgnoreCase(COMPASS_HEADING, headingStr);
            if (ndx >= 0) {
                return ndx * COMPASS_INCREMENT;
            } else {
    ...
    private void plainTextResponse(final HttpServletResponse response, final String errMsg)
        throws ServletException, IOException
    {
    ...
    protected static void logError(final String URL, final String msg)
    {
        if (URL != null) {
    ...
    protected static void logException(final String URL, final String msg, final Throwable th)
    {
        if (URL != null) {
            Print.logError(LOG_NAME + ": " + URL);
        }
