...
public abstract class SMSOutboundGateway
{
    ...
    **/
    public static void _startupInit()
    {
        ...
        SMSOutboundGateway.AddSMSGateway(GW_clickatell, new SMSOutboundGateway() {
            public DCServerFactory.ResultCode sendSMSCommand(Device device, String commandStr) {
              ...
              //String toEmail  = this.getSmsEmailAddress(device);
                ...
                // ---
                String message  = commandStr;
                int maxLen = RTConfig.getInt(PROP_clickatell_maxMessageLength, GetDefaultMaximumMessageLength());
                if (StringTools.length(message) > maxLen) {
                    // exceeds maximum allowed SMS text length
                    Print.logWarn("Invalid SMS text length: " + StringTools.length(message));
                }
        ...
        /* EMail: TextAnywhere [ENTERPRISE] [deprecated - do not use] */
        // -- deprecated: support for TextAnywhere may be removed in a future release
        // Properties:
        ...
        SMSOutboundGateway.AddSMSGateway(GW_mail2txt, new SMSOutboundGateway() {
            public DCServerFactory.ResultCode sendSMSCommand(Device device, String commandStr) {
                ...
                // ---
                String message = commandStr;
                int maxLen = RTConfig.getInt(PROP_mail2txt_maxMessageLength, GetDefaultMaximumMessageLength());
                if (StringTools.length(message) > maxLen) {
                    // exceeds maximum allowed SMS text length
                    Print.logWarn("Invalid SMS text length: " + StringTools.length(message));
                }
        ...
        SMSOutboundGateway.AddSMSGateway(GW_mail2txt160, new SMSOutboundGateway() {
            public DCServerFactory.ResultCode sendSMSCommand(Device device, String commandStr) {
                ...
                // ---
                String message = commandStr;
                int maxLen = RTConfig.getInt(PROP_mail2txt160_maxMessageLength, GetDefaultMaximumMessageLength());
                if (StringTools.length(message) > maxLen) {
                    // exceeds maximum allowed SMS text length
                    Print.logWarn("Invalid SMS text length: " + StringTools.length(message));
                }
        ...
        SMSOutboundGateway.AddSMSGateway(GW_mail2txtid, new SMSOutboundGateway() {
            public DCServerFactory.ResultCode sendSMSCommand(Device device, String commandStr) {
                ...
                // ---
                String message  = commandStr + "$$";
                int maxLen = RTConfig.getInt(PROP_mail2txtid_maxMessageLength, GetDefaultMaximumMessageLength());
                if (StringTools.length(message) > maxLen) {
                    // exceeds maximum allowed SMS text length
                    Print.logWarn("Invalid SMS text length: " + StringTools.length(message));
                }
        ...
        SMSOutboundGateway.AddSMSGateway(GW_mail2txt160id, new SMSOutboundGateway() {
            public DCServerFactory.ResultCode sendSMSCommand(Device device, String commandStr) {
                ...
                // ---
                String message  = commandStr + "$$";
                int maxLen = RTConfig.getInt(PROP_mail2txt160id_maxMessageLength, GetDefaultMaximumMessageLength());
                if (StringTools.length(message) > maxLen) {
                    // -- exceeds maximum allowed SMS text length
                    Print.logWarn("Invalid SMS text length: " + StringTools.length(message));
                }
        ...
        SMSOutboundGateway.AddSMSGateway(GW_ozekisms, new SMSOutboundGateway() {
            public DCServerFactory.ResultCode sendSMSCommand(Device device, String commandStr) {
                ...
                // ---
                String message = commandStr;
                int maxLen = RTConfig.getInt(PROP_ozekisms_maxMessageLength, GetDefaultMaximumMessageLength());
                if (StringTools.length(message) > maxLen) {
                    // -- exceeds maximum allowed SMS text length
                    Print.logWarn("Invalid SMS text length: " + StringTools.length(message));
                }
        ...
        /* HTTP: Twilio [ENTERPRISE] NOT YET SUPPORTED */
        // -- Custom implementation is needed here to complete the Twilio outbound SMS messaing support
        // -  Twilio has its own support library for Java, which can be found at the following link:
        ...
        {
            String gwName = GW_twilio;
            String gwKey  = PROP_twilio_className;
        ...
        /* HTTP: Aeris [ENTERPRISE] NOT YET SUPPORTED */
        {
            String gwName = GW_aeris;
            String gwKey  = PROP_aeris_className;
            String gwClassName = RTConfig.getString(gwKey,"");
            if (!StringTools.isBlank(gwClassName)) {
                try {
                    Class gwClass = Class.forName(gwClassName); // ClassNotFoundException
