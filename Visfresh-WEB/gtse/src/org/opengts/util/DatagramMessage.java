...
public class DatagramMessage
{
    ...
    **/
    public static void main(String argv[])
    {
        ...
        /* UDP stress test */
        // -- provides UDP packet stress testing
        if (RTConfig.hasProperty(ARG_STRESSTEST)) {
            if (port <= 0) {
                if (cmdRecv) {
                    Print.logError("Receive bind port not specified");
                } else {
                    Print.logError("Destination port not specified");
                }
                usage();
            }
            if (!cmdRecv && StringTools.isBlank(host)) {
                Print.logError("Destination host not specified");
                usage();
            }
            // --
            String SEQ_SEP = "]";
            ...
            try {
                if (cmdRecv) {
                    // -- receive
                    // -  bin/exeJava org.opengts.util.DatagramMessage -udpStressTest -port=37000 -recv
                    Print.sysPrintln("Receiving UDP test data: " + port);
                    ...
                    for (;;) {
                        // -- receive UDP packet
                        byte recvB[] = dgm.receive(60*1000);
                        if ((recvB == null) || (recvB.length == 0)) {
                            Print.sysPrintln("\nNull/Empty UDP packet received");
                            continue;
                        }
                        pktCnt++;
                        byteCnt += recvB.length;
                        String recvS = StringTools.toStringValue(recvB);
                        // -- extract sequence number
                        int p = recvS.indexOf(SEQ_SEP);
                        if ((p <= 0) || (p > 6)) {
                            Print.sysPrintln("");
                            Print.sysPrintln("Recv: " + recvS);
                            Print.sysPrintln("ERROR: Received packet does not contain a sequence number");
                            break;
                        }
                        // -- check sequence number
                        int seq = StringTools.parseInt(recvS, -2);
                        if (seq <= lastSeq) {
                            // -- starting over?
                            Print.sysPrintln("");
                            Print.sysPrintln("Starting over with sequence: " + seq);
                            startMS = DateTime.getCurrentTimeMillis();
                            lastMS  = -1L;
                        } else
                        if ((lastSeq >= 0) && (seq != (lastSeq + 1))) {
                            // -- missing (lastSeq+1)..(seq-1)
                            int cnt = seq - lastSeq - 1;
                            dropCnt += cnt;
                            Print.sysPrintln("");
                            Print.sysPrintln("Dropped UDP packet: seq="+(lastSeq+1)+"/"+cnt+", total="+dropCnt);
                        }
                        lastSeq = seq;
                        // -- display progress indicator
                        long nowMS = DateTime.getCurrentTimeMillis();
                        if (lastMS <= 0L) {
                            Print.sysPrintln("First packet: " + recvS);
                            lastMS  = nowMS;
                            startMS = DateTime.getCurrentTimeMillis(); // start timer
                        } else
                        if ((nowMS - lastMS) >= 10000L) {
                            double pps = ((double)pktCnt / (double)(nowMS - startMS)) * 1000.0; // (pkt/ms)*(ms/sec)
                            Print.sysPrintln("Summary: #packets="+pktCnt+", packets/second="+pps);
                            lastMS  = nowMS;
                          //startMS = nowMS;
                        //} else
                    ...
                    // -- send
                    // -  bin/exeJava org.opengts.util.DatagramMessage -udpStressTest=20,60,400 -host=localhost -port=37000 
                    int udpTest[] = RTConfig.getIntArray(ARG_STRESSTEST, null);
                    if (ListTools.size(udpTest) < 2) {
                        Print.logError("Invalid '"+ARG_STRESSTEST[0]+"' option in 'send' mode: "+RTConfig.getString(ARG_STRESSTEST,null));
                        usage();
                    }
                    long   msgPerSec = (udpTest.length > 0)? Math.min(Math.max(udpTest[0], 1), 200) :  20; //  1 <= M <=  200 : messages/second
                    long   seconds   = (udpTest.length > 1)? Math.min(Math.max(udpTest[1], 1), 900) :  60; //  1 <= S <=  900 : seconds
                    int    packetLen = (udpTest.length > 2)? Math.min(Math.max(udpTest[2],20),1500) : 400; // 20 <= P <= 1500 : packet length
                    long   count     = msgPerSec * seconds;
                    long   sequence  = 0L;
                    long   delayMS   = 1000L / msgPerSec;
                    long   stopTime  = DateTime.getCurrentTimeSec() + seconds;
                    String sendS     = StringTools.replicateString("0123456789",(packetLen+9)/10);
                    Print.sysPrintln("Sending UDP test to: " + host + ":" + port + " ["+msgPerSec+"m/s, "+seconds+" sec]");
                    dgm = new DatagramMessage(host, port, -1, null);
                    long lastSec = -1L;
                    for (;;) {
                        //if ((sequence % 300L) == 0L) { sequence++; } // <-- used for debug purposes to force a missing sequence
                        byte sendB[] = ((sequence++) + SEQ_SEP + sendS).getBytes();
                        dgm.send(sendB);
