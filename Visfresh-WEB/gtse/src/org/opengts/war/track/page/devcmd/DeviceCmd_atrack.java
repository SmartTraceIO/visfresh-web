...
//     -Initial release (cloned from "rvct03")
// ----------------------------------------------------------------------------
    ...
    implements DeviceCmdHandler
{
    ...
    /* device code */
    public static final String  DEVICE_CODE                 = DCServerFactory.ATRACK_NAME;
        ...
        throws IOException
    {
        ...
        I18N         i18n        = privLabel.getI18N(DeviceCmd_atrack.class);
        ...
        out.println(DeviceInfo.FormRow_TextField(PARM_DEVICE  , false, i18n.getString("DeviceCmd_atrack.deviceID","{0} ID",devTitles)+":"            , selDevID, 30, 30));
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_DESC, false, i18n.getString("DeviceCmd_atrack.deviceDesc","{0} Description",devTitles) +":", (selDev!=null)?selDev.getDescription():"", 40, 40));
        ...
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_LAST_CONNECT, false, i18n.getString("DeviceCmd_atrack.lastCommunication","Last Communication") +":", lastCommTime , 30, 30));
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_LAST_EVENT  , false, i18n.getString("DeviceCmd_atrack.lastEvent","Last Event") +":"                , lastEventTime, 30, 30));

        /* commands */
        out.println("<tr class='deviceCommandList'>");
        out.println("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap style=\"vertical-align:top; padding-top:5px;\">"+i18n.getString("DeviceCmd_atrack.commandSelect","Command Select")+":</td>");
        ...
        if (dcs != null) {
            ...
            if (!ListTools.isEmpty(cmdMap_admin)) {
                cmdCount += this._writeCommands(out, reqState, 
                    i18n.getString("DeviceCmd_atrack.standardCommands","Standard Commands"),
            ...
            if (!ListTools.isEmpty(cmdMap_sysadmin)) {
                cmdCount += this._writeCommands(out, reqState, 
                    i18n.getString("DeviceCmd_atrack.sysadminCommands","SysAdmin Commands"),
        ...
        if (cmdCount == 0) {
            ...
            out.print("<span>"+i18n.getString("DeviceCmd_atrack.noCommandsAvailable","No Commands Available")+"</span>");
            out.print("</td>");
            out.print("</tr>\n");
        }
        ...
        out.println(DeviceInfo.FormRow_ComboBox(PARM_COMMAND_SELECT , editProps, i18n.getString("DeviceCmd_atrack.commands","Command")+":", commandSel, commandMap, "", -1));
        */
        ...
        if (editProps) {
            out.write("<input type='submit' name='"+PARM_SUBMIT_SEND+"' value='"+i18n.getString("DeviceCmd_atrack.send","Send")+"'>\n");
            out.write("<span style='padding-left:10px'>&nbsp;</span>\n");
            out.write("<input type='button' name='"+PARM_BUTTON_CANCEL+"' value='"+i18n.getString("DeviceCmd_atrack.cancel","Cancel")+"' onclick=\"javascript:openURL('"+actionURL+"','_self');\">\n");
        } else {
            out.write("<input type='button' name='"+PARM_BUTTON_BACK+"' value='"+i18n.getString("DeviceCmd_atrack.back","Back")+"' onclick=\"javascript:openURL('"+actionURL+"','_self');\">\n");
        }
        out.write("</form>\n");
        return true;
        ...
        Map<String,DCServerConfig.Command> cmdMap, boolean editProps, I18N i18n)
    {
        ...
        if (!ListTools.isEmpty(cmdMap)) {
            for (DCServerConfig.Command cmd : cmdMap.values()) {
                String cmdID     = cmd.getName();                                                               // getName()
                String radioID   = RADIO_CMD_SEL_ + cmdID;
                String radioDesc = cmd.getDescription();                                                        // getDescription()
                boolean hasArgs  = cmd.hasCommandArgs();                                                        // hasCommandArgs()
                ...
                if (hasArgs) {
                    ...
                    for (int i = 0; i < argCnt; i++) {
                        DCServerConfig.CommandArg cmdArg = cmd.getCommandArg(i);                                // getCommandArg(i)
                        String radioTextName = RADIO_CMD_TEXT_ + cmdID + "_" + i;
                        String argDesc = (cmdArg != null)? cmdArg.getDescription() : (i > 0)? ("Arg"+i) : "";   // getCommandArg(i).getDescription()
                        if (!StringTools.isBlank(argDesc)) {
                            out.print("<div class='"+CSS_deviceCommandArgDiv+"'>"+argDesc+": ");
                        } else {
                            out.println(": ");
                        }
                        String value  = (cmdArg != null)? cmdArg.getDefaultValue() : "";                        // getCommandArg(i).getDefaultValue()
                        String resKey = (cmdArg != null)? cmdArg.getResourceName() : null;                      // getCommandArg(i).getResourceName()
                        if (!StringTools.isBlank(resKey)) {
                            value = StringTools.trim(AttributeTools.getSessionAttribute(request,resKey,value));
                        }
                        int dispLen = (cmdArg != null)? cmdArg.getDisplayLength() :  70;                        // getCommandArg(i).getDisplayLength()
                        int maxLen  = (cmdArg != null)? cmdArg.getMaximumLength() : 200;                        // getCommandArg(i).getMaximumLength()
                        if ((cmdArg != null) && cmdArg.isReadOnly()) {                                          // getCommandArg(i).isReadOnly()
    ...
    {
        if (resp == null) {
            return i18n.getString("DeviceCmd_atrack.unableToQueueCommand","Unable to queue command for transmission");
        } else {
            Print.logInfo("Response: " + resp);
            DCServerFactory.ResultCode rc = DCServerFactory.getCommandResultCode(resp); // not null
            String m = rc.getMessage((i18n != null)? i18n.getLocale() : null);
            if ((rc == null) || rc.isSuccess()) {
                return i18n.getString("DeviceCmd_atrack.commandQueued","Requested command has been queued for transmission");
            } else {
                return i18n.getString("DeviceCmd_atrack.commandError" ,"Unable to queue command: {0}", m);
            }
        }
    }

    /* send selected command to device */
    public String handleDeviceCommands(RequestProperties reqState, Device selDev)
    {
        ...
        /* supported device? */
        if (!this.deviceSupportsCommands(selDev)) {
            return i18n.getString("DeviceCmd_atrack.doesNotSupport","Device does not support {0}", this.getServerDescription());
        }

        /* get selected command */
        String cmdSel = AttributeTools.getRequestString(request, PARM_COMMAND_SELECT, "");
        if (StringTools.isBlank(cmdSel)) {
            return i18n.getString("DeviceCmd_atrack.noCommandSelected","No command selected");
        }
        ...
        */
        ...
        /* unknown command */
        return i18n.getString("DeviceCmd_atrack.unknownCommand","No command selected");
