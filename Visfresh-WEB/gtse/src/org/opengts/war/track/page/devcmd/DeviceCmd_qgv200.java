...
//     -Initial release (cloned from qgv100)
// ----------------------------------------------------------------------------
    ...
    implements DeviceCmdHandler
{
    ...
    /* device code */
    public static final String  DEVICE_CODE                 = DCServerFactory.QGV200_NAME;
        ...
        throws IOException
    {
        ...
        I18N         i18n        = privLabel.getI18N(DeviceCmd_qgv200.class);
        ...
        out.println(DeviceInfo.FormRow_TextField(PARM_DEVICE  , false, i18n.getString("DeviceCmd_qgv200.deviceID","{0} ID",devTitles)+":"            , selDevID, 30, 30));
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_DESC, false, i18n.getString("DeviceCmd_qgv200.deviceDesc","{0} Description",devTitles) +":", (selDev!=null)?selDev.getDescription():"", 40, 40));
        ...
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_LAST_CONNECT, false, i18n.getString("DeviceCmd_qgv200.lastCommunication","Last Communication") +":", lastCommTime , 30, 30));
        out.println(DeviceInfo.FormRow_TextField(PARM_DEV_LAST_EVENT  , false, i18n.getString("DeviceCmd_qgv200.lastEvent","Last Event") +":"                , lastEventTime, 30, 30));

        /* commands */
        out.println("<tr class='deviceCommandList'>");
        out.println("<td class='"+CommonServlet.CSS_ADMIN_VIEW_TABLE_HEADER+"' nowrap style=\"vertical-align:top; padding-top:5px;\">"+i18n.getString("DeviceCmd_qgv200.commandSelect","Command Select")+":</td>");
        ...
        if (dcs != null) {
            ...
            if (!ListTools.isEmpty(cmdMap_admin)) {
                cmdCount += this._writeCommands(out, reqState, 
                    i18n.getString("DeviceCmd_qgv200.standardCommands","Standard Commands"),
            ...
            if (!ListTools.isEmpty(cmdMap_sysadmin)) {
                cmdCount += this._writeCommands(out, reqState, 
                    i18n.getString("DeviceCmd_qgv200.sysadminCommands","SysAdmin Commands"),
        ...
        if (cmdCount == 0) {
            ...
            out.print("<span>"+i18n.getString("DeviceCmd_qgv200.noCommandsAvailable","No Commands Available")+"</span>");
            out.print("</td>");
            out.print("</tr>\n");
        }
        ...
        out.println(DeviceInfo.FormRow_ComboBox(PARM_COMMAND_SELECT , editProps, i18n.getString("DeviceCmd_qgv200.commands","Command")+":", commandSel, commandMap, "", -1));
        */
        ...
        if (editProps) {
            out.write("<input type='submit' name='"+PARM_SUBMIT_SEND+"' value='"+i18n.getString("DeviceCmd_qgv200.send","Send")+"'>\n");
            out.write("<span style='padding-left:10px'>&nbsp;</span>\n");
            out.write("<input type='button' name='"+PARM_BUTTON_CANCEL+"' value='"+i18n.getString("DeviceCmd_qgv200.cancel","Cancel")+"' onclick=\"javascript:openURL('"+actionURL+"','_self');\">\n");
        } else {
            out.write("<input type='button' name='"+PARM_BUTTON_BACK+"' value='"+i18n.getString("DeviceCmd_qgv200.back","Back")+"' onclick=\"javascript:openURL('"+actionURL+"','_self');\">\n");
        }
        out.write("</form>\n");
        return true;
    ...
    {
        if (resp == null) {
            return i18n.getString("DeviceCmd_qgv200.unableToQueueCommand","Unable to queue command for transmission");
        } else {
            ...
            if ((rc == null) || rc.isSuccess()) {
                return i18n.getString("DeviceCmd_qgv200.commandQueued","Requested command has been queued for transmission");
            } else {
                return i18n.getString("DeviceCmd_qgv200.commandError" ,"Unable to queue command: {0}", m);
            }
        }
    }
    ...
    public String handleDeviceCommands(RequestProperties reqState, Device selDev)
    {
        ...
        /* supported device? */
        if (!this.deviceSupportsCommands(selDev)) {
            return i18n.getString("DeviceCmd_qgv200.doesNotSupport","Device does not support {0}", this.getServerDescription());
        }

        /* get selected command */
        String cmdSel = AttributeTools.getRequestString(request, PARM_COMMAND_SELECT, "");
        if (StringTools.isBlank(cmdSel)) {
            return i18n.getString("DeviceCmd_qgv200.noCommandSelected","No command selected");
        }
        ...
        */
        ...
        /* unknown command */
        return i18n.getString("DeviceCmd_qgv200.unknownCommand","No command selected");
