...
public class MapPanel extends JPanel {
    ...
    private final MapImageBuilder mapBuilder = new MapImageBuilder();
    ...
    public Point.Double getLongitudeLatitude(final Point position) {
        return new Point.Double(
                MapImageBuilder.position2lon(position.x, getZoom()),
                MapImageBuilder.position2lat(position.y, getZoom()));
    }

    public Point computePosition(final Point.Double coords) {
        final int x = MapImageBuilder.lon2position(coords.x, getZoom());
        final int y = MapImageBuilder.lat2position(coords.y, getZoom());
    ...
    private static final class Painter {
        ...
        private Painter(final MapPanel mapPanel, final int zoom) {
            this.mapPanel = mapPanel;
            this.zoom = zoom;
        }

        private void paint(final Graphics2D gOrig, final Point mapPosition) {
            try {
                mapPanel.mapBuilder.paint(gOrig, mapPosition,
                        zoom, mapPanel.getWidth(), mapPanel.getHeight());
    ...
    private void paintInternal(final Graphics2D g) {
        ...
        final Painter painter = new Painter(this, getZoom());
        painter.paint(g, position);
    }
    ...
    public final class OverlayPanel extends JPanel {
        ...
        private void paintOverlay(final Graphics2D g) {
            ...
            drawString(g, 8, "Cursor Lon/Lat", format(
                    MapImageBuilder.position2lon(getCursorPosition().x, getZoom())) + ", " + format(
                            MapImageBuilder.position2lat(getCursorPosition().y, getZoom())));
        }
